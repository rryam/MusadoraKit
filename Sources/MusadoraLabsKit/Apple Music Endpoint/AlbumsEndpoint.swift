...
extension AppleMusicEndpoint {
    ...
    static func catalogAlbum(id: String, storeFront: String? = nil) async throws -> URL {
        try await AppleMusicEndpoint(library: .catalog, path: MusicItemPath.albums.id(id), storeFront: storeFront).url
    }
    ...
    static func catalogAlbums(ids: [String], storeFront: String? = nil) async throws -> URL {
        ...
        return try await AppleMusicEndpoint(library: .catalog, path: .albums, storeFront: storeFront, queryItems: [query]).url
    }
    ...
    static func libraryAlbum(id: String) async throws -> URL {
        try await AppleMusicEndpoint(library: .library, path: MusicItemPath.albums.id(id)).url
    }

    static func libraryAlbums(ids: [String]) async throws -> URL {
        let query = URLQueryItem(name: "ids", value: ids.joined(separator: ","))
        return try await AppleMusicEndpoint(library: .library, path: .albums, queryItems: [query]).url
    }

    static func libraryAlbums() async throws -> URL {
        try await AppleMusicEndpoint(library: .library, path: .albums).url
    }
}
