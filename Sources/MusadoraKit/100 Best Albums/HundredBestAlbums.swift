...
public extension MRecommendation {
  ...
  static func hundredBestAlbum(at position: Int) async throws -> HundredBestAlbum {
    let request = HundredBestAlbumRequest(position: position)
  ...
  static func allHundredBestAlbums() async throws -> [HundredBestAlbum] {
    let positions = 1...100
    ...
    return try await withThrowingTaskGroup(of: (Int, HundredBestAlbum?).self) { group in
      for position in positions {
        group.addTask {
          do {
            let album = try await self.hundredBestAlbum(at: position)
